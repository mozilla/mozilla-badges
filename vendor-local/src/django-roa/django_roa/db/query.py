...
class RemoteQuerySet(query.QuerySet):
    ...
    def get(self, *args, **kwargs):
        ...
        if filters.keys() == ['id']:
            return self._get_from_id_or_pk(id=filters['id'])
        # useful for admin which relies on PKs
        elif filters.keys() == ['pk']:
            return self._get_from_id_or_pk(pk=filters['pk'])
        # check the case of PK attribute with custom name
        elif filters.keys() == [custom_pk]:
            return self._get_from_id_or_pk(pk=filters[custom_pk])
        # check if there's an exact match filter
        elif len(exact_match) == 1:
            # use the value of exact match filter to retrieve object by PK
            return self._get_from_id_or_pk(pk=filters[exact_match[0]])
        else:
